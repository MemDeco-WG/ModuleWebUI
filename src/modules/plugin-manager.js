...
// 使用 import.meta.glob 让构建器在打包阶段预解析并收集插件文件（若目录存在）
const PLUGIN_IMPORTS =
  typeof import.meta !== "undefined" && import.meta.glob
    ? import.meta.glob("../plugins/**/*.{js,ts,mjs}")
    : {};
...
class PluginManager {
  ...
  async loadPluginFromConfig(config) {
    ...
    try {
      ...
      const importPath = `../plugins/${config.id}/${entry}`;
      let pluginModule = null;
      if (PLUGIN_IMPORTS && PLUGIN_IMPORTS[importPath]) {
        pluginModule = await PLUGIN_IMPORTS[importPath]();
      } else if (typeof import.meta !== "undefined" && import.meta.glob) {
        // 运行时尝试使用 import.meta.glob 获取导入函数（若构建阶段未被拾取）
        const importers = import.meta.glob("../plugins/**/*.{js,ts,mjs}");
        if (importPath in importers) {
          pluginModule = await importers[importPath]();
        } else {
          pluginModule = await import(/* @vite-ignore */ importPath);
        }
      } else {
        // 最后回退，使用动态导入（vite-ignore 以避免在构建时解析路径）
        pluginModule = await import(/* @vite-ignore */ importPath);
      }
  ...
  async _loadPluginLegacy(pluginId) {
    ...
    try {
      // 动态导入插件（优先使用 PLUGIN_IMPORTS 映射）
      const pluginPath = `../plugins/${pluginId}/index.js`;
      let pluginModule = null;
      if (PLUGIN_IMPORTS && PLUGIN_IMPORTS[pluginPath]) {
        pluginModule = await PLUGIN_IMPORTS[pluginPath]();
      } else if (typeof import.meta !== "undefined" && import.meta.glob) {
        const importers = import.meta.glob("../plugins/**/*.{js,ts,mjs}");
        if (pluginPath in importers) {
          pluginModule = await importers[pluginPath]();
        } else {
